...
// Groups a flat release list into one entry per system, newest-first within each —
// N systems can publish releases here, not just the LED-panel app (the first one).
function groupBySystem(releases: Release[]): { systemKey: string; systemLabel: string; releases: Release[] }[] {
  const order: string[] = [];
  const groups: Record<string, Release[]> = {};
  for (const r of releases) {
    if (!groups[r.systemKey]) { groups[r.systemKey] = []; order.push(r.systemKey); }
    groups[r.systemKey].push(r);
  }
  return order.map(systemKey => ({
    systemKey,
    systemLabel: groups[systemKey][0].systemLabel || systemKey,
    releases: groups[systemKey],
  }));
}
...
export default function DownloadsPage() {
  ...
  async function download(systemKey: string) {
    const res = await fetch(`/api/v2/devices/latest-version?system=${encodeURIComponent(systemKey)}`);
    const data = await res.json();
    if (data.downloadUrl) window.location.href = data.downloadUrl;
  }

  const systems = groupBySystem(releases);
      ...
      <div className="max-w-3xl mx-auto py-8 px-4 space-y-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Instaláveis dos apps de desktop do sistema. Depois de instalado, cada app verifica e baixa
        ...
        {loading ? (
          ...
          systems.map(({ systemKey, systemLabel, releases: systemReleases }) => {
            const latest = systemReleases[0];
            return (
              <div key={systemKey} className="space-y-3">
