...
async function main() {
  ...
  const events = await (prisma as any).event.findMany({
    where: {
      checkoutAt: null,
      OR: [{ teardownAt: { gte: today } }, { startAt: { gte: today } }],
    },
    include: { contracts: { orderBy: { createdAt: 'asc' }, select: { id: true, externalId: true } } },
  });

  console.log(`Eventos candidatos (futuros, sem checkoutAt, com contrato): ${events.filter((e: any) => e.contracts.length > 0).length} de ${events.length} futuros totais`);
  ...
  for (const ev of events) {
    ...
    try {
      ...
      const checkoutAt = parseBrt(detail?.main?.data_checkout);
      if (!checkoutAt) { skippedNoCheckout++; continue; }
      await (prisma as any).event.update({ where: { id: ev.id }, data: { checkoutAt } });
      updated++;
      console.log(`OK  ${ev.name} -> checkoutAt=${checkoutAt.toISOString()}`);
    } catch (e: any) {
      errors++;
  ...
  console.log(`\nResumo: ${updated} atualizados, ${skippedNoContract} sem contrato, ${skippedNoCheckout} sem data_checkout no UERP, ${errors} erros.`);
